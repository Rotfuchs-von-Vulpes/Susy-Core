...
//            new Etchant(HydrofluoricAcid, TitaniumNitride, 50, 700, false),
    ...
    private static void removeCuttingRecipe(MetaItem<?>.MetaValueItem item) {
        removeRecipesByInputs(CUTTER_RECIPES, new ItemStack[] {item.getStackForm()}, new FluidStack[]{Lubricant.getFluid(84)});
        removeRecipesByInputs(CUTTER_RECIPES, new ItemStack[] {item.getStackForm()}, new FluidStack[]{DistilledWater.getFluid(253)});
        removeRecipesByInputs(CUTTER_RECIPES, new ItemStack[] {item.getStackForm()}, new FluidStack[]{Water.getFluid(337)});
    }
    public static void init() {
        //REMOVALS

// CPU * 8
        removeCuttingRecipe(CENTRAL_PROCESSING_UNIT_WAFER);
// RAM * 32
        removeCuttingRecipe(RANDOM_ACCESS_MEMORY_WAFER);
// ULPIC * 6
        removeCuttingRecipe(ULTRA_LOW_POWER_INTEGRATED_CIRCUIT_WAFER);
// LPIC * 4
        removeCuttingRecipe(LOW_POWER_INTEGRATED_CIRCUIT_WAFER);
// Integrated Circuit * 8
        removeCuttingRecipe(INTEGRATED_LOGIC_CIRCUIT_WAFER);
// PIC * 4
        removeCuttingRecipe(POWER_INTEGRATED_CIRCUIT_WAFER);
// NAND * 32
        removeCuttingRecipe(NAND_MEMORY_CHIP_WAFER);
// NOR * 16
        removeCuttingRecipe(NOR_MEMORY_CHIP_WAFER);
